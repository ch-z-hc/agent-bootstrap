...
`provider add` 会交互填写显示名、Base URL、API 类型、API key 和 model。model 默认会请求兼容 OpenAI 的 `GET /models` 自动发现，也可以选择纯手动输入，或在自动发现后补充自定义 model。删除仍被 agent 使用的 provider 会被拒绝，确认改绑后再使用 `--force`。

`provider refresh <id>` 会重新查询并替换该 provider 的 model 列表；加上 `--merge` 则只追加 / 更新查询结果，不删除现有 model。

向导会先让你确认 Claude Code、Codex、Pi、DSH 的配置文件路径（自动探测到的现有文件会作为默认值），再逐个编辑 YAML 中已有的 provider。`gpt` 默认从自定义入口开始，适合代理或自建网关；DeepSeek、OpenAI、OpenRouter 和 SiliconFlow 的地址预设只会在对应的 provider ID 下显示，不会出现在通用的 `gpt` provider 中。provider 不会被强制合并或删除，agent 也会继续使用各自原来的 provider。随后显示 model 目录，可按编号勾选保留；最后逐项显示各 agent 的 model 选择。API key 可以直接输入并保存到本机 YAML；输入 API key 时直接回车可保留已有值。确认后同步所有 agent。路径会写入 YAML 的 `paths` 节点，Windows / Linux 可以分别保存自己的路径。
...
model 列表始终按 provider 隔离：来自 YAML 的当前 provider model 会保留；`gpt` 还会合并 Codex 本地 `models.json`（如果存在），`opencode-go` 使用自己的内置常见 model 目录。CLI 不会把另一个已配置 provider 的 model 复制到 `gpt`。需要注意的是，Codex 官方允许自定义 provider ID 与模型名称不一致，因此一个代理网关如果主动返回 `deepseek-*` model，仍会按网关实际返回结果显示；这不代表 CLI 把 DeepSeek 配置串到了 `gpt`。

Codex 的 provider 语义以官方 `model_providers.<id>` 配置为准：<https://developers.openai.com/codex/config-reference/>。
...
从 [`agent-vendors.example.yaml`](agent-vendors.example.yaml) 开始。`providers` 定义 provider 和 model，`agents` 只负责选择 provider/model 以及目标特有选项。API key 直接填写在本机 YAML 中即可，示例中的 key 均为占位符。`baseURL` 是 OpenAI 兼容接口地址；需要 Claude Code 时可在同一 provider 下配置 `anthropicBaseURL`（也支持 `endpoints.anthropic-messages`），这样同一份凭据和 model 不会因为协议不同而串地址。OpenCode Go 未配置该字段时会自动将 `/v1` 去掉作为 Anthropic 地址。
...
Claude Code 当前通过 `opencode-go` provider 写入模型设置；Codex 使用 `gpt`，Pi / DSH / ZCode 会按 YAML 中的 provider 和 model 同步。
